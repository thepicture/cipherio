...
module.exports = {
  encode: (text, seed, options) => {
    const codes = getCharCodesFromSource(text);

    const encoded = `${encode(text, codes)}${JSON.stringify(
      Object.fromEntries(codes.entries())
    )}`;

    if (options?.compressed) {
      return getCharCodeShiftedText(encoded.replaceAll('"', ""), seed);
    }

    return getCharCodeShiftedText(encoded, seed);
  },
  decode: (encoded, seed, options) => {
    let [, binary, codes] = /([\d,]+)({.+})/.exec(
      getCharCodeShiftedText(encoded, -seed)
    );

    if (options?.compressed) {
      // {a:1,b:2} => {"a":"1","b":"2"}
      codes = codes.replace(/([^,:{}]+):([^,:{}]+)/g, '"$1":"$2"');
    }

    const decoded = decode(
      binary.split(","),
      new Map(Object.entries(JSON.parse(codes)))
    );
